...
# 
abalone_clean <- abalone |>   #new data frame from abalone dataset
  # cleaned names with underscores and no caps
  clean_names() |> 
  # select only mean_ph and change_in_area columns
...
#| message: false
# base layer ggplot
ggplot(data = abalone_clean, # data is abalone clean
       aes(x = mean_ph, # x axis is mean pH (predictor)
           y = change_in_area)) + #y axis is change in area (response)
  # add in scatter plot layer
  geom_point(size = 4, #set size of points
             stroke = 1,
             fill = "firebrick4", #change color of points
             shape = 21)
```
...
**With each increase in pH, the model predicts an increase of 3.0 +/- 0.76 in change in shell surface are per day (mm^2^ d^-1^)**  

What is the intercept estimate (with standard error?)  

**when pH is zero, the model predicts that abalone change in shell surface area per day is -18.6. (Note to self: this is not realistic but is something the model generates)**
...
**x is the predictor variable values for pH**
**predicted is the model predictions for change in shell surface area per day**
**st. error: standard error describing precision in model predictions**
...
**At ph = 8, the model predicts that abalone change in shell surface area per day should be 5.36 mm^2^ d^-1^ (95% CI: [5.08, 5.64])**
